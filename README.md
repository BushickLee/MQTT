...
| --- | --- | --- |
| `fall-guard-app` | Edge/model/backend 쪽 위험 이벤트 생산자 또는 backend 소비자 | 모델 결과에 `event_id`, `frame_id`, `timestamp`, `object_type`, `object_type_ko`, `hls_url`을 보강해 `risk/alerts/raw`에 publish |
...
최신 front의 `RiskObjectType`과 `fall-guard-app` furniture schema에 맞춰 `object_type`은 `chair`, `sofa`, `table`, `bed` 중 하나만 허용합니다. `confidence`는 `0` 이상 `1` 이하입니다. MQTT alert는 가장 높은 `phase`와 `confidence`만 전달합니다.
